...
func (config *MixConfig) SaveConfig(filename string) error {
	...
	filename, err := GetConfigPath(filename)
	if err != nil {
		return err
	}
...
func (config *MixConfig) legacyParse(filename string) error {
	...
	fields := []struct {
		...
		// [Server]
		{`^DEBUG_INFO_BANNED\s*=\s*`, &config.Server.DebugInfoBanned, false},
		{`^DEBUG_INFO_LIB\s*=\s*`, &config.Server.DebugInfoLib, false},
		{`^DEBUG_INFO_SRC\s*=\s*`, &config.Server.DebugInfoSrc, false},
		// [Mixer]
